...
from utils.approximate_search import single_emb_search, double_emb_search_01
...
#        self.image_display.setPixmap(QtGui.QPixmap.fromImage(image))
    ...
    def _display_output(self, output_number):
        ...
        bbox2 = self.output_list['bbox2'].iloc[output_number]
        ...
        query_img = cv2.rectangle(query_img,(bbox2[0],bbox2[1]), (bbox2[2],bbox2[3]), (255,0,255), 4)
    ...
    def run_single_search(self):
        ...
        if check_file == -1:
            ...
            process_terminal_output += "File2 Does not Exist or Address Invalid \n"
            self.process_terminal.setText(process_terminal_output)
        elif check_file2 == 0:
            process_terminal_output += "File2 format not support. Use one of the following image formats: \n"
            buff_format = ' '
            buff_format = buff_format.join(supported_image_formats)
            process_terminal_output += buff_format
            process_terminal_output += '\n'
            ...
            # Display query image with bounding box using QPixmap
            query_img = cv2.cvtColor(cv2.imread(file_addr2), cv2.COLOR_RGB2BGR)
            height, width, channel = query_img.shape
            bytesPerLine = 3 * width
            # Convert cv2Image to QImage
            qImg = QtGui.QImage(query_img.data, width, height, bytesPerLine, QtGui.QImage.Format_RGB888)
            ...
            #self.input_image_pixmap.scaled(64, 64, QtCore.Qt.KeepAspectRatio)
            # Display Image
            self.input_image_display2.setPixmap(self.input_image_pixmap.scaled(480, 480, QtCore.Qt.KeepAspectRatio))
            ...
            face_emb2, bbox2, counter2 = self.detector.get_face_emb(file_addr2)
            ...
            if counter2 == -1:
                process_terminal_output += "No face detected in Image2 \n Terminating \n"
                self.process_terminal.setText(process_terminal_output)
                return 0
            ...
            self.output_list = double_emb_search_01(file_name, face_emb, face_emb2, num_results=100)
            #self.output_list = single_emb_search(file_name, face_emb, num_results=100)
    ...
    def _create_central_widget(self):
        ...
        # Inilialize and Build File Picker
        self.file_prompt2 = dir_file_picker(file_type=1)
        layout_0.addWidget(self.file_prompt2.basic_widget)
        ...
        #
        #
        self.input_display = QHBoxLayout()
        # Black Place-holder to display input image
        self.input_image_display = QLabel('Input1')
        self.input_display.addWidget(self.input_image_display, alignment=Qt.AlignCenter)
        # Black Place-holder to display input image
        self.input_image_display2 = QLabel('Input2')
        self.input_display.addWidget(self.input_image_display2, alignment=Qt.AlignCenter)

        # Add Display Layout to layout_01
        layout_01.addLayout(self.input_display)
